...
function getExistingApplicant($student_id)
{
...
function getLastApplicationId()
		{
			...
			$sql = "SELECT MAX(application_id) as result FROM application";
			$result = $connect->query($sql);
...
function createApplication($application_id, $applicant_id, $job_id)
{
	$connect= new DBConnection();
	$connect = $connect->getInstance();
	...
	$sql = "INSERT INTO application(application_id, applicant_id, job_id, date, time, place, notes, decision, decision_message) 
	VALUES ('$application_id', '$applicant_id', '$job_id', '', '', '', '', '', '')";
