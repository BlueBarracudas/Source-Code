...
function deactivateAccount($account_id)
{
	...
	$sql = "UPDATE account SET isactive='0' WHERE account_id='" . $account_id . "'"; 
        ...
        function insertCompanyWorkExp($job_id)
        {
            $connect= new DBConnection();
	        $connect = $connect->getInstance();
            ...
            if(isset($_POST['jobtitle']) && isset($_POST['workExp']))
             {
              ...
              for($i=0;$i<$cnt;$i++)
              {
                  $query="INSERT INTO `jobit`.`workexpjoblisting`(`job_id`,`title`,`years`)VALUES('$job_id','$job[$i]','$year[$i]');";
        ...
        function insertCertificate($job_id)
        {
            $connect= new DBConnection();
	        $connect = $connect->getInstance();

             if(isset($_POST['certification']) && isset($_POST['radioBtn']))
            {
                $name = array();
                $name = $_POST['certification'];
                $date = array();
                $complecency = $_POST['radioBtn'];
                $cnt=count($name);
                $i = 0;
                 
              for($i=0;$i<$cnt;$i++){
                  if(strcmp($complecency[$i],"yes")){
                      $comp = 1;
                  }else{
                      $comp = 0;
                  }
                 $query="INSERT INTO `jobit`.`certjoblisting`(`job_id`,`name`,`competency`)VALUES('$job_id','$name[$i]','$comp');";
        ...
        function deleteJobListing($job_id)
        {
            $connect= new DBConnection();
			$connect = $connect->getInstance();
            echo "whut";
            $del = "DELETE FROM `jobit`.`joblisting`
WHERE '$job_id';";
            echo $del;
        ...
        function getLastJobID()
		{
			...
			$sql = "SELECT MAX(job_id) as result FROM joblisting";
			$result = $connect->query($sql);
        ...
        function createJobListing($company_id, $position, $location, $course, $hours, $day, $work, $cert, $pdf, $slots)
        {
            $connect= new DBConnection();
			$connect = $connect->getInstance();
            
			$sql = "INSERT INTO `jobit`.`joblisting`(`company_id`,`position`,`location`,`day`,
`hours`,`course`,`slots_available`,`joblist_pdf`)VALUES('$company_id','$position','$location','$day','$hours','$course','$slots','$pdf');";
            ...
            insertCompanyWorkExp($job_id);
            insertCertificate($job_id);

            $weresult = mysqli_query($connect, "SELECT * FROM `jobit`.`workexpjoblisting` WHERE job_id = '$job_id';");
            ...
            $certresult = mysqli_query($connect, "SELECT * FROM `jobit`.`certjoblisting` WHERE job_id = '$job_id';");
            ...
            if(!(empty($cert)) && !(empty($we)))
            {
                $up = "UPDATE `jobit`.`joblisting` SET `exp_tag` = '$we',`cert_tag` = '$cert' WHERE `job_id` = '$job_id';";
                  if ($connect->query($up) !== TRUE) {
                        echo "ERROR: Could not able to execute $sql. " . mysqli_error($connect);
                  }
                
            }
            else if((empty($cert)) && !(empty($we)))
            {
                $up = "UPDATE `jobit`.`joblisting` SET `exp_tag` = '$we' WHERE `job_id` = '$job_id';";
                  if ($connect->query($up) !== TRUE) {
                        echo "ERROR: Could not able to execute $sql. " . mysqli_error($connect);
                  }
            ...
            {
                $up = "UPDATE `jobit`.`joblisting` SET `cert_tag` = '$cert' WHERE `job_id` = '$job_id';";
                  if ($connect->query($up) !== TRUE) {
                      echo "ERROR: Could not able to execute $sql. " . mysqli_error($connect);
                  }
